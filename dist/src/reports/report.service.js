...
let ReportService = class ReportService {
    ...
    constructor(opportunityRepository, funnelStageRepository) {
        this.opportunityRepository = opportunityRepository;
        this.funnelStageRepository = funnelStageRepository;
    }
    async getFunnelData(funnel_id = 25, period = 'month', dateRange) {
        let startDate;
        let endDate;
        ...
        return stages
            ...
            const conversionRate = previousCount > 0 ? Math.round((currentCount / previousCount) * 100) : null;
            if (currentCount > 0) {
    ...
    async getOwnerPerformance(funnel_id = 25, period = 'month', dateRange) {
        let startDate;
        let endDate;
        const today = new Date();
        if (dateRange) {
        ...
        const opportunities = await this.opportunityRepository.find({
            where: {
                is_deleted: false,
                funnel_id,
                created_at: (0, typeorm_3.Between)(startDate, endDate),
            },
            relations: ['owner'],
        });
        const stats = opportunities.reduce((acc, opp) => {
            ...
            if (opp.is_closed && opp.close_date) {
                ...
                if (opp.created_at) {
                    const durationInDays = Math.floor((new Date(opp.close_date).getTime() - new Date(opp.created_at).getTime()) /
                        (1000 * 60 * 60 * 24));
        ...
        const result = Object.values(stats).map(item => {
            const avgDuration = item.closeDurations.length > 0
            ...
            return {
                ...
                winRate: item.totalDeals > 0 ? Math.round((item.wonDeals / item.totalDeals) * 100) : 0,
                totalRevenue: item.totalRevenue,
                avgDealValue: item.wonDeals > 0 ? Math.round(item.totalRevenue / item.wonDeals) : 0,
                avgCloseTime: Number(avgDuration.toFixed(1)),
            };
        });
        return result;
    }
    async getRevenueReport(funnel_id = 25, period = 'month', dateRange) {
        let startDate;
        let endDate;
        const today = new Date();
        if (dateRange) {
        ...
        const deals = await this.opportunityRepository.find({
            where: {
                ...
                funnel_id,
                is_closed: true,
                close_date: (0, typeorm_3.Between)(startDate, endDate),
            },
    ...
    async getDealActivityTrend(funnel_id = 25, period = 'month', dateRange) {
        let startDate;
        let endDate;
        const today = new Date();
        if (dateRange) {
    ...
    async exportFunnelSheet(funnel_id = 25, period = 'month', dateRange) {
        const funnelData = await this.getFunnelData(funnel_id, period, dateRange);
        const sheet = XLSX.utils.json_to_sheet(funnelData.map(d => ({
            Этап: d.stage_name,
            Сделки: d.count,
    ...
    async exportPerformanceSheet(funnel_id = 25, period = 'month', dateRange) {
        const performanceData = await this.getOwnerPerformance(funnel_id, period, dateRange);
        const sheet = XLSX.utils.json_to_sheet(performanceData.map(d => ({
    ...
    async exportFunnelWithConversionToExcel(funnel_id = 25, period = 'month', dateRange) {
        const funnelData = await this.getFunnelData(funnel_id, period, dateRange);
        const sheet = XLSX.utils.json_to_sheet(funnelData.map(d => ({
            Этап: d.stage_name,
            Сделки: d.count,
    ...
    async exportRevenueReportToExcel(funnel_id = 25, period = 'month', dateRange) {
    ...
    async exportDealActivityToExcel(funnel_id = 25, period = 'month', dateRange) {
        const reportData = await this.getDealActivityTrend(funnel_id, period, dateRange);
        const activitySheet = XLSX.utils.json_to_sheet(reportData.periods.map(p => ({
            Период: p.label,
            'Всего сделок': p.total,
            Изменение: `${p.changeFromPrevious}%`,
        })));
        const topPerformersSheet = XLSX.utils.json_to_sheet(reportData.summary.topPerformers.map(t => ({
